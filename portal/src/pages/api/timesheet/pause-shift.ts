...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    await pauseActiveShift(employee.id);
    ...
    return new Response(null, {
      status: 303,
      headers: { Location: resolveTimesheetReturnTo(form, '/timesheet?paused=1') },
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unable to pause your shift.';
