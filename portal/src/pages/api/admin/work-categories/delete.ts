...
import { deactivateWorkCategory } from '../../../../lib/timesheet-work-categories';
...
export const POST: APIRoute = async ({ locals, request }) => {
  ...
  try {
    await deactivateWorkCategory(String(form.get('id') ?? '').trim());
  } catch (error) {
    return workCategoriesAdminError(error instanceof Error ? error.message : 'Could not remove work item.');
  }
